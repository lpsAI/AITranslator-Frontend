...
export const ChatHeader = () => {
  ...
  useEffect(() => {
    const reqOne = axios.get(
      "https://ai-translator-backend.vercel.app/api/v1/getLang",
      {
        headers: {
    ...
    const reqTwo = axios.get(
      "https://ai-translator-backend.vercel.app/api/v1/getLocaleLang",
      {
        headers: {
          "X-Custom-Header": "foobar",
        },
    ...
    const initAllLanguages = async () => {
      axios.all([reqOne, reqTwo]).then(
        axios.spread((...res) => {
          res[0].data.languages.sort((a, b) =>
            a.langName.localeCompare(b.langName)
          );
          res[1].data.languageLocale.sort((a, b) =>
          ...
          localStorage.setItem(
            "list_languages",
            JSON.stringify(res[0].data.languages)
          );
          localStorage.setItem(
            "locale_list",
            JSON.stringify(res[1].data.languageLocale)
          );
  ...
  return (
    <>
      <div className="drawer">
        ...
        <div className="drawer-content">
          <div className="navbar bg-base-300 sticky top-0">
            <div className="navbar-start">
              <a className="max-sm:hidden text-lg font-bold">
                Welcome {currentUser.email ?? user.email}!
              </a>
              <a
                className="max-sm:hidden text-md m-2 text-center cursor-pointer"
            ...
            <div className="navbar-center">
              <a className="text-lg font-bold mx-6">LPS AI Translator</a>
            </div>
            <div className="navbar-end">
              <div className="flex items-stretch">
                <ul className="menu menu-horizontal px-1">
                  <li>
                    <Link
                      className="max-sm:hidden text-lg m-2 text-center"
                      to={"/"}
                    >
                      Chat
                    </Link>
                  ...
                  <li>
                    ...
                    >
                      Language <b>{localStorage.getItem("language")}</b>
                    </a>
                  </li>
                  <li>
                    <button
                      className="max-sm:hidden text-lg m-2 text-center w-12 h-19 hidden sm:inline"
                      color="gray"
                      onClick={toggleTheme}
                    >
                  ...
                  <li>
                    <label
                      htmlFor="my-drawer"
                      className="btn mx-3 drawer-button sm:hidden mr-20"
                    >
                      ☰
                    </label>
                  </li>
        ...
        <div className="drawer-side">
          ...
          <ul className="justify-center menu p-4 w-60 min-h-full bg-base-200 text-base-content">
            ...
            <li
              className=" text-md m-2 text-center cursor-pointer"
              onClick={() => handleLogout()}
            >
              Logout
            </li>
            <li>
              <Link className="text-lg m-2 text-center" to={"/"}>
                Chat
              </Link>
            </li>
            <li>
              <Link className="text-lg m-2 text-center" to={"/voice"}>
                Voice
              </Link>
            </li>
            <li>
              <Link className="text-lg m-2 text-center" to={"/image"}>
                Image
              </Link>
            </li>
            <li>
            ...
            <li>
              <button
                className="text-lg m-2 text-center w-12 h-19 inline"
                color="gray"
                onClick={toggleTheme}
              >
                {theme === "light" ? <SunIcon /> : <MoonIcon />}
      ...
      <Modalbase toOpen={() => handleCloseModal()} isModalOpen={toClose}>
        ...
        {handleSearchFilter(languageList).length != 0 ? (
          <ul tabIndex={0} className="container flex flex-wrap justify-between">
          ...
          <p className="text-center text-lg">No language available</p>
        )}
      </Modalbase>
    </>
  );
